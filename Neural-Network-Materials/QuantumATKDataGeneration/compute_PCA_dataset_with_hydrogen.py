...
dos_threshold_index = len(error) #np.where(error <= 1e-4)[0][0]
print("The index at threshold 1e-4: ", len(error))
fig, ax = plt.subplots()
ax.plot(error)
...
# Creating a new dataset to replace the new one and calculate means
...
for ii in range(len(silicon_dataset)):
    atoms, properties = silicon_dataset.get_properties(ii)
    ...
    if (ii in train_idx):  # Calculating the mean and std for training data points
...
silicon_dataset = AtomsData(dataset_filename, available_properties=properties_list[0].keys())
