...
def plot_vinj():
    ...
    if rank == 0:
        plt.figure()
        plt.plot(energies, smooth_positive_Jx)
...
iterate_vals = list(os.listdir(read_dir))
if rank == 0:
...
for file in iterator:
    ...
    if dos_values.shape[0] == 2:
        assert np.sum(dos_values[0] - dos_values[1]) < 1e-20
    ...
    if rank == 0:
        ...
        property_dict = {Properties.neighbors: neighborhood_idx, "neighbors": neighborhood_idx, "cell_offset": offset,
                         ...
                         "lat_vol": lat_area, "Ec": Ec, "Ef": fermi_level}
...
if rank == 0:
    ...
    silicon_dataset = AtomsData(dataset_filename, available_properties=property_dict.keys())
    silicon_dataset.add_systems(atoms_list, property_list)

    ''' Generating the training, validation and test data '''

    num_train = 9 * num_total // 10
    num_val = num_total // 20

    idx = np.random.permutation(num_total)
    train_idx = idx[0:num_train].tolist()
    val_idx = idx[num_train: num_train + num_val].tolist()
    test_idx = idx[num_train + num_val:].tolist()
    np.savez(hyperparameter_split_filename, train_idx=train_idx, val_idx=val_idx, test_idx=test_idx)
