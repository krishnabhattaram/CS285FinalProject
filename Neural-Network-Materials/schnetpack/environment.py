...
class BondEnvironmentProvider(BaseEnvironmentProvider):
    """
    Environment provider making use of bond indices given as input. Supports PBCs.

    """

    def __init__(self, cutoff = 5.0, neighbor_string = "neighbors", offset_string = "cell_offset"):
        self.cutoff = cutoff
    ...
    def get_environment(self, atoms, grid=None):
        ...
        if self.neighbor_string not in atoms.info['data'].keys():
            raise Exception('Neighbor indices not provided in the database')
        if self.offset_string not in atoms.info['data'].keys():
            raise Exception('Cell offset not provided in the database')
        #if Properties.neighbor_mask not in atoms.info['data'].keys():
        #    raise Exception('Neighbor mask not provided in the database')
             
    
        n_atoms = atoms.get_global_number_of_atoms()
        n_neigh_max = atoms.info['data'][self.neighbor_string].shape[1]
        neighborhood_idx = atoms.info['data'][self.neighbor_string]
        #neighborhood_mask = atoms.info['data'][Properties.neighbor_mask]  
        offset = atoms.info['data'][self.offset_string]
