...
class MLP(nn.Module):
    ...
    def relprop(self,
        ...
        inputs_list = list(reversed(inputs_list))
        for ii, nn in enumerate(reversed(list(self.out_net))):
            self.relevance.append(nn.relprop(inputs_list[ii + 1], self.relevance[ii], gamma = gamma, epsilon = epsilon))
        
        self.relevance = list(reversed(self.relevance))[0:-1]
        
        print(self.relevance)
        return self.relevance[0]    # Returning the relevance of the input to MLP
