...
def atom_distances(
    ...
    dist_vec = pos_xyz - positions[:, :, None, :] - 1e-6    # So that NaNs don't appear during backward gradients. Subtract so that it doesn't get out of cutoff
    ...
    if return_vecs:
        ...
        if normalize_vecs:
            dist_vec = dist_vec / (tmp_distances[:, :, :, None])    # To avoid Nans
...
class AtomVectors(nn.Module):
    ...
    def __init__(self):
        super(AtomVectors, self).__init__()
        self.return_directions = True
    ...
    def forward(
        ...
        return atom_distances(
            ...
            normalize_vecs=True,
            neighbor_mask=neighbor_mask,
        )[1]          # Returning only the distance vectors
...
def triple_distances(
    ...
    offset_idx_j:Optional[torch.Tensor]=None,
    offset_idx_k:Optional[torch.Tensor]=None,
    cell:Optional[torch.Tensor]=None,
    cell_offsets:Optional[torch.Tensor]=None,
    return_vecs:bool = False
) -> Tuple[torch.Tensor]:
    ...
    Args:
        ...
        cell_offsets (torch.Tensor, optional): offset of atom in cell coordinates
            with (N_b x N_at x N_nbh x 3) shape.
        return_vecs (Bool): Where to return the direction vectors or not
...
class TriplesDistances(nn.Module):
    ...
    def forward(self,
                positions:torch.Tensor,
                neighbors_j:torch.Tensor,
                neighbors_k:torch.Tensor) -> Tuple[torch.Tensor]:
        """
        Args:
            positions (torch.Tensor): Atomic positions
            neighbors_j (torch.Tensor): Indices of first neighbor in triangle
...
def neighbor_elements(atomic_numbers:torch.Tensor,
