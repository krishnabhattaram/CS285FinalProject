...
__all__ = [
    ...
    "calc_gradient_clip_norm",
]


def convert_dict_to_class(input_dict):
...
def calc_gradient_clip_norm(model, fraction = 0.05):
    """
    Utility function to calculate the value for gradient clipping by norm. This is done by 
    gradient clip norm = fraction* weight norm

    Args:
        model (schnetpack.atomistic.AtomisticModel): SchNetPack model. Our main compltete neural network
        fraction (float): fraction of the total weight norm for calculation gradient clip norm val
        
    Returns:
        gradient_clip_norm (Optional, float = 0.05) : The value of gradient clipping by norm
    """
    
    weight_norm = 0.0
    for p in model.parameters():
        weight_norm += p.data.norm(2)**2
    weight_norm = weight_norm**(1/2)
    gradient_clip_norm = (fraction*weight_norm).round()
    
    print('The value of the weight norm : ', weight_norm)
    print('The value of gradient clip norm : ', gradient_clip_norm)
