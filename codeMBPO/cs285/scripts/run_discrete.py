...
def run_training_loop(
    ...
    # initialize agent
    dos_model = DOSModelSchnet(
        env,
        num_dos=200,
        **config["agent_kwargs"],
    )

    print(f'DOS Model has {sum([np.prod(p.size()) for p in dos_model])} parameters')
    ...
    dqn_replay_buffer = ReplayBuffer(dqn_config["replay_buffer_capacity"])
    ...
    for itr in tqdm.tqdm(range(config["num_iters"])):
        ...
        # if dqn_config is not None:
        for traj in trajs:
            dqn_replay_buffer.batched_insert(
                observations=traj["observation"],
                actions=traj["action"],
                rewards=traj["reward"],
                next_observations=traj["next_observation"],
        ...
        dos_model.update_statistics(
            ...
            dos=env.get_dos(replay_buffer.observations[: len(replay_buffer)]),
        )
        ...
        for _ in tqdm.trange(
            ...
            if isinstance(dos_model, DOSModelSchnet):
                ...
                step_losses += [dos_model.update(i=0, obs=transitions["observations"],
                                                 dos=env.get_dos(transitions["observations"]))]
            else:
        ...
        if itr > -1:
            ...
            obs = env.reset()[None, :]
            dos_obs = env.get_dos(obs).squeeze()
        ...
        for i in tqdm.trange(
            ...
            # train SAC
            batch = dqn_replay_buffer.sample(dqn_config["batch_size"])
            dqn_agent.update(
