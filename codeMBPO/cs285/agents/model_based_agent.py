...
class ModelBasedAgent(nn.Module):
    ...
    def update(self, i: int, obs: np.ndarray, acs: np.ndarray, next_obs: np.ndarray):
        ...
        predicted_norm_obs_delta = self.dynamics_models[i](torch.hstack((obs, acs)))

        loss = self.loss_fn(norm_obs_delta, predicted_norm_obs_delta)
    ...
    def update_statistics(self, obs: np.ndarray, acs: np.ndarray, next_obs: np.ndarray):
        ...
        self.obs_acs_std, self.obs_acs_mean = torch.std_mean(acs, dim=0, keepdim=True)
        self.obs_delta_std, self.obs_delta_mean = torch.std_mean(next_obs - obs, dim=0, keepdim=True)
    ...
    def get_dynamics_predictions(
        ...
        # normalizing inputs!
        predicted_norm_obs_delta = self.dynamics_models[i](torch.hstack((obs, acs)))
        predicted_obs_delta = (predicted_norm_obs_delta + self.obs_delta_mean) * self.obs_delta_std
        pred_next_obs = obs + predicted_obs_delta
    ...
    def evaluate_action_sequences(self, obs: np.ndarray, action_sequences: np.ndarray):
        ...
        for acs in np.transpose(action_sequences, (1, 0, 2)):
            ...
            rewards = self.env.get_reward(next_obs.reshape((-1, self.ob_dim)),
                                          acs.reshape((-1, self.ac_dim)))[0].reshape((self.ensemble_size,
                                                                                      self.mpc_num_action_sequences))
    ...
    def get_action(self, obs: np.ndarray):
        ...
        if self.mpc_strategy == "random":
            ...
            for i in range(self.cem_num_iters):
                ...
                elite_actions = action_sequences[np.argsort(rewards)[-self.cem_num_elites:]]
                ...
                action_sequences = np.random.normal(elite_mean, elite_std, size=(self.mpc_num_action_sequences,
                                                                                 self.mpc_horizon,
                                                                                 self.ac_dim))
