...
class SoftActorCritic(nn.Module):
    ...
    def q_backup_strategy(self, next_qs: torch.Tensor) -> torch.Tensor:
        ...
        if self.target_critic_backup_type == "doubleq":
            ...
            next_qs_temp = next_qs[0].clone()
            next_qs[0] = next_qs[1].clone()
            next_qs[1] = next_qs_temp
        elif self.target_critic_backup_type == "min":
            assert num_critic_networks == 2

            next_qs_v = torch.fmin(next_qs[0], next_qs[1])
            next_qs[0] = next_qs[1] = next_qs_v
        elif self.target_critic_backup_type == "redq":
            target_0, target_1 = np.random.choice(num_critic_networks, 2, replace=False)

            next_qs_v = torch.fmin(next_qs[target_0], next_qs[target_1])
            next_qs = next_qs_v.repeat(num_critic_networks, 1)
        else:
    ...
    def update_critic(
        ...
        with torch.no_grad():
            ...
            # next_qs = self.q_backup_strategy(next_qs)  # Not sure why this is called twice??
            ...
            target_values: torch.Tensor = reward + self.discount * next_qs * (1.0 - done.int())
    ...
    def actor_loss_reinforce(self, obs: torch.Tensor):
        ...
        with torch.no_grad():
            ...
            q_values = self.critic(obs=obs.repeat(self.num_actor_samples, 1, 1), action=action)
    ...
    def update(
        ...
        for _ in range(self.num_critic_updates):
            critic_infos += [self.update_critic(obs=observations, action=actions, reward=rewards,
                                                next_obs=next_observations, done=dones)]
        ...
        assert bool(self.target_update_period is not None) != bool(self.soft_target_update_rate is not None)
